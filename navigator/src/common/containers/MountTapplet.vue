<template>
  <Page>
    <PageContent>
      <v-card class="pa-0">
        <v-card-title
          class="pb-3 card-title type--flex"
          name="tapplet-title"
        >
        ...
        <v-card-text v-else>
          ...
          <br>The Tapplet file needs to be a <b>.zip</b> that contains at
          least an <b>index.html</b> file.
        </v-card-text>
      </v-card>

      <div
        v-if="tapplets && tapplets.length > 0"
        class="mt-8 pl-3 pr-3"
      >
...
export default class MountTapplet extends Vue {
  ...
  unsupported =
    this.platform === "browser" || this.platform === "cordova browser";
