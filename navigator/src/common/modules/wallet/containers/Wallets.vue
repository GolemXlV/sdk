<template>
  <Page>
    <PageContent
      :width="500"
      :flex="walletsByParentKey.length < 1"
    >
      <div
        v-if="walletsByParentKey.length < 1"
        class="box ma-auto"
      >
      ...
      <v-card
        v-for="group of walletsByParentKey"
        :key="group.id"
        class="mb-3"
      >
        <v-subheader class="px-4">
          ...
          <v-btn
            ...
            @click="importWallet(group.key.id)"
          >
            <v-icon
              left
              class="mr-1"
          ...
          >
            <template v-slot:activator="{ on }">
              <v-btn
                icon
                class="half-transparent mr-n1 ml-2"
                small
                v-on="on"
              >
...
export default class WalletCreated extends Vue {
  ...
  copy(value: string) {
    const { dispatch } = store;
    store.dispatch.Common.copy(value).catch(() =>
      console.warn("Couldn't copy")
    );
  }
