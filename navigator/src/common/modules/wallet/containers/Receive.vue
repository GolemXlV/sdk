      ...
      <v-tabs-items v-model="tab">
        ...
        <v-tab-item key="Two">
          ...
          <v-list-item-subtitle
            class="mb-2 slightly-transparent"
          >
            Invoice URL
          </v-list-item-subtitle>
          <v-text-field
      ...
      <v-row dense>
        <v-col class="py-0">
          <v-btn
            ...
            large
            @click="
              form.amount !== null ? downloadInvoiceQR() : downloadAddressQR()
            "
          >
...
export default class Wallet extends Vue {
  ...
  downloadInvoiceQR(
    qr = this.invoiceQr,
    name = `${this.wallet.name}-invoice.png`
  ) {
    const { dispatch } = store;
    dispatch.Common.downloadURL({ url: qr, name });
  }
  ...
  get error() {
    ...
    if (amount !== null && touched) {
      if (comment.length > maxCommentSize)
        errors.comment.push(
          `Comment can't be longer than ${maxCommentSize} characters`
        );
    }
