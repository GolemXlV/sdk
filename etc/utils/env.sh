...
if [ "$DEPLOYTYPE" == "metal" ]; then
  ...
  if ! [ -n "$IMAGEURI" ]; then IMAGEURI="registry.gitlab.com/gram-net/gram:1.0.0"; fi
  NEW_IMAGEURI=
...
echo "FIFTLIB=${FIFTLIB}" >> $GRAMCORE/.env
...
cp $GRAMCORE/docker-compose-template.yml $GRAMCORE/docker-compose.yml
python $ETC/lib/subst.py --props $GRAMCORE/.env --src_path $GRAMCORE/docker-compose-template.yml --dst_path $GRAMCORE/docker-compose.yml
