...
cd $PROFILE
$BUILDDIR/utils/generate-initial-keys -i ${IP}:${TON_ENGINE_PORT}
cd ..
...
$BUILDDIR/utils/generate-random-id -m keys -n $PROFILE/keys/validator_temp > $PROFILE/keys/validator_temp.out
$BUILDDIR/utils/generate-random-id -m keys -n $PROFILE/keys/validator_adnl > $PROFILE/keys/validator_adnl.out
$BUILDDIR/utils/generate-random-id -m keys -n $PROFILE/keys/client > $PROFILE/keys/client.out
$BUILDDIR/utils/generate-random-id -m keys -n $PROFILE/keys/liteserver > $PROFILE/keys/liteserver.out
$BUILDDIR/utils/generate-random-id -m keys -n $PROFILE/keys/server > $PROFILE/keys/server.out
...
ec $ADDRLISTJSON
$BUILDDIR/utils/generate-random-id -k $PROFILE/keys/auto_dht.pk -m dht -a "${ADDRLISTJSON}" > $PROFILE/dht-node 
