...
export type filesExp = Array<{ encoding?: string, path?: string, name: string, data: ArrayBuffer | string | unknown }>
...
export class Wasm {
  ...
  constructor(wasmSrc:any, wasmFile?:ArrayBuffer, asmBin?:ArrayBuffer, memInit?:ArrayBuffer, crypto?:any) {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  async exec(files: filesExp, args:Array<string>=[], outFiles:Array<string>=[], interactiveCmds?:Array<string>) {
    if(this.running) {
    ...
    this.execPromise = new Promise( (resolve, reject) => {
      ...
      this.wasmObj = this.wasmSrc({
        ...
        waitCmd: async (flags:number) => {
          ...
          try {
            await until(async (resolve: any, reject: any) => {
              if(!this.cmdRunning && this.currentCmd) {
                // console.log('%c%s','background:white;color:black',"JS yielded back to C++ STDIN")
                // console.log(this.currentCmd)                
                resolve(true)
              }
            },sleepNumber,120000,"waitCmd")
          } catch(e) {
          ...
          try{
            ...
            for (const x in interactiveCmds) {
              ...
              try {
                ...
                console.log('%cCMD RESULT: '+JSON.stringify(result),'font-size:14pt;color:lightgreen;')    
              }
              catch(result) {
        ...
        performance: {now: function(){const res=new Date();return res.getTime()}},
        onAbort: (x:any) => { this.onAbort(x) },
        onExit: (x:any) => { this.onExit(x) },
        print: (x:string) => { this.print(x)},
        printErr: (x:string) => {  this.printErr(x)},
      })
    });
  ...
  printErr(str:string) {
    ...
    if(str.indexOf('exited (with status: ') > -1) {
      console.log('%cprintERR %s','color:green;font-size:9pt;font-style:italic;',"Exit Status Code",str);
      if(this.isInteractive) { this.onExit(69) }
    } else {
  ...
  print(str:string) {
    console.log('%c%s','font-size:9pt;font-style:italic;background:black;color:olive',str);
  ...
  examineResult() {
    console.log('%c%s','background-color:green;padding:5px;color:white;font-size:13pt;',"EXAMINING RESULT")
    if(this.log.length) { 
      console.log('%c%s','font-size:9pt;font-style:italic;background:black;color:olive',this.log.join('\n'));
     }
    if(!this.log.length || this.err.length) {
      ...
      if(this.err.length) { 
        console.log('%c%s','font-size:9pt;font-style:italic;background:black;color:hotpink',this.err.join('\n'));
      }
    }
  }
  processOutfiles() {
    if(this.outFiles) {
      try {
        for(let i=0;i<this.outFiles.length;i++) {
          ...
          const fileBuffer = this.wasmObj.FS.readFile(this.outFiles[i], { encoding: 'binary' })
  ...
  writeFiles(wasmObj:any) {
    for(const i in this.files) {
      ...
      if(this.files[i].encoding === 'binary') {
        wasmObj.FS.writeFile("/"+thisFile.name, new Int8Array(thisFile.data as any), {encoding:'binary'});
      } else {
...
export class Fift extends Wasm {
  constructor(wasmFile?: ArrayBuffer, asmBin?:ArrayBuffer, memInit?: ArrayBuffer, crypto?: any) {
    super((asmBin ? fiftfastcompasync : (memInit ? fiftfastcomp : fift)), wasmFile, asmBin, memInit, crypto)
...
export class Func extends Wasm {
  // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  constructor(wasmFile?: ArrayBuffer, asmBin?:ArrayBuffer, memInit?:ArrayBuffer, crypto?: any) {
    super((asmBin ? funcfastcompasync : (memInit ? funcfastcomp : func)), wasmFile, asmBin, memInit, crypto)
