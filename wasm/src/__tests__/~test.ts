...
const style3 = 'text-transform:uppercase;color:red;font-size:14pt;padding:10px;background:black;'
...
export class Test {
  ...
  async runTest(tests:string|Array<string>) {
    ...
    if(!group || !battery) {
      throw new Error("running multiple tests from a test type is not yet supported. use full name IE: fift-interactive-success")
    }
    ...
    if(typeof testType[group] !== 'object' || group == 'parent') {
      throw new Error("test group "+group+" is not an object or uses reserved word 'parent'")
    }    
