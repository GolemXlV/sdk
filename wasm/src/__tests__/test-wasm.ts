  ...
  fift = class {
    ...
    script = {
      fail: async () => {
        ...
        await this.parent.imports.fiftObj.exec(scriptFiles, scriptArgs, ['test-wallet.pk', 'test-wallet-query.boc', 'test-wallet.addr'])
      },
      success: async () => {
        const scriptFiles = [{
          name: "new-wallet.fif",
    ...
    interactive = {
      ...
      success: async () =>{
        await this.parent.imports.fiftObj.exec([{name:'newwallet.fif',data:FIXT.newWallet}], [], [], 
        [
          '<b 123456789 32 u, b> <s dup csr.',
          'dup .s',
          'dup csr.',
          'quit'
        ])    
