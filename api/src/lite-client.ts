...
export class LiteClient {
  ...
  async sendfile(content:Uint8Array) { 
    const fn = join(<string>process.env.PROFILE, 't'+Buffer.from(nacl.randomBytes(32)).toString('base64').replace(/\//g,'-'))
  ...
  async runmethod(address:string, method:string) {
    return <string[]> await this.query(`runmethod ${address} ${method}`,
  ...
  async query(q:string, handle:any) {
    ...
    const w:ChildProcessWithoutNullStreams = this.idle.length ? (this.idle.pop() as ChildProcessWithoutNullStreams) : await this.createWorker()
