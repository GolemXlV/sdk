...
export default [{
  ...
  handler: async (request: Request, response: Response) => {
    ...
    if (!account) {
      ...
      const resultAccTrans: Account = await callCppBackend(urlStructAcc).catch(error => { console.log(error) }) as Account
      try {
        if (resultAccTrans && resultAccTrans.last_transaction) {
          const transaction = resultAccTrans.last_transaction
          const urlStruct = '/transactions?account=' + transaction.account + '&lt=' + transaction.lt + '&hash=' + transaction.hash
          console.log('urlStruct', urlStruct)
          code = 200
          result = await callCppBackend(urlStruct).catch(error => { console.log(error) })
        } else {
          code = 404
