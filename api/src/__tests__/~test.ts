...
async function testTonkit () {
  ...
  const result4 = await axios(`${ENDPOINT}runmethod?method=seqno&address=${testgiverAddr}`)
  console.warn('seqno: from api', result4.data)
  const result5 = await axios(`${ENDPOINT}runmethod?method=seqno&address=${testgiverAddr}1`)
  console.warn('seqno: bad address', result5.data)
  const result6 = await axios(`${ENDPOINT}runmethod?method=badmethod&address=${testgiverAddr}`)
